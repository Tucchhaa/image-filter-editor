...
function getColorFromPalette(value: number, palette: string): [number, number, number] {
    switch (palette) {
        case "psychedelic":
            return [
        ...
        case "retro":
            return [
                Math.cos(value * 0.5) * 127 + 200,
                Math.sin(value * 0.4) * 127 + 200,
                Math.cos(value * 0.3) * 127 + 200,
            ];
        case "firestorm":
            return [
        ...
        for (let x = 1; x < width - 1; x++) {
            ...
            for (let ky = -1; ky <= 1; ky++) {
                for (let kx = -1; kx <= 1; kx++) {
                    ...
                    const intensity = (getPixel(x + kx, y + ky, 0) +
                        getPixel(x + kx, y + ky, 1) +
                        getPixel(x + kx, y + ky, 2)) / 3;
    ...
    Options: ({ setOptions }) => {
        ...
        return (
            <div>
                <label>
                    Color Palette:
                    <select name="palette" onChange={handleInputChange}>
